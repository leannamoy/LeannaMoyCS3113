...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("Enemy AI",
...
void process_input()
{
    ...
    //S to deflect
        ...
        g_current_scene->m_state.player->set_shield(true);
    }
...
bool checkGameWin() {
    for (int i = 0; i < g_current_scene->m_state.ENEMY_COUNT; i++) {
        if (g_current_scene->m_state.enemies[i].get_active() == true) return false;
    }
    return true;
}
...
void render()
{
    ...
    if (g_current_scene->m_state.player->get_active() == false) {
        Utility::draw_text(&g_shader_program, g_text_texture_id, std::string("You Lose!"), 0.25f, 0.0f, g_current_scene->m_state.player->get_position() + glm::vec3(0.0f, 1.0f, 0.0f));

    }

    Utility::draw_text(&g_shader_program, g_text_texture_id, std::string("Press 'S' to deflect the projectile"), 0.25f, 0.0f, glm::vec3(1.0f, -1.0f, 0.0f));
    Utility::draw_text(&g_shader_program, g_text_texture_id, std::string("back at the shooter."), 0.25f, 0.0f, glm::vec3(1.0f, -1.25f, 0.0f));

    if (checkGameWin() == true) {
        Utility::draw_text(&g_shader_program, g_text_texture_id, std::string("You Win!"), 0.25f, 0.0f, g_current_scene->m_state.player->get_position()+glm::vec3(0.0f, 1.0f, 0.0f));
